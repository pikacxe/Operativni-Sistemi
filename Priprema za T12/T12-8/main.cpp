/*
Napraviti konkurentan program koji formira najbolju ponudu cena od dva zadata cenovnika (A i B). 
Cenovnici se zadaju uz pomoć map kontejnera, pri čemu ključ predstavlja naziv proizvoda (string) a podatak je cena proizvoda (double).

Program treba da podrazumeva da oba cenovnika (dve mape) sigurno sadrže iste proizvode (svaki proizvod se sigurno pojavljuje u obe). 
Samo se deo sa cenama razlikuje. Program treba da u novoj niti obavi ispitivanje - da pronađe nižu od dve cene i da je ubaci u rezultujući cenovnik.
Nebitno je u kojoj radnji je nađen proizvod, rezultujući cenovnik (kao i dva ulazna) sadrži samo naziv proizvoda i cenu.
